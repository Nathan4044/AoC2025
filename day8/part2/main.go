...
func main() {
	...
	last := constructChain(distances, boxes)

	fmt.Println(last.left.x * last.right.x)
}

func constructChain(distances []distance, boxes []*box) *distance {
	currentChain := 1
